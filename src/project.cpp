...
#undef SKIP_WS
	...
	// NOTE: отладочный вывод при необходимости можно и раскомментить
	/*
...
static bool
modesForGenerator(const QString &gen, Option::HOST_MODE *host_mode, Option::TARG_MODE *target_mode)
{
	if (gen == "UNIX") {
#ifdef Q_OS_MAC
...
{
	if (Option::host_mode == Option::HOST_UNKNOWN_MODE || Option::target_mode == Option::TARG_UNKNOWN_MODE)
	{
		...
		else if (modesForGenerator(gen.first(), &host_mode, &target_mode))
		{
